...
export const queryNextTransType = async ({
  ...
  try {
    const query = `{
      translatableResources(resourceType: ${resourceType}, first: 2 ${endCursor ? `, after: "${endCursor}"` : ""}) {
        nodes {
          resourceId
          translatableContent {
            key
...
export const queryPreviousTransType = async ({
  ...
  try {
    const query = `{
      translatableResources(resourceType: ${resourceType}, last: 2 ${startCursor ? `, before: "${startCursor}"` : ""}) {
        nodes {
          resourceId
          translatableContent {
            key
