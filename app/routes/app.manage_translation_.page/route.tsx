  ...
  if (refreshResourceIds.length > 0) {
    try {
      const response = await admin.graphql(
        `#graphql
          query refreshPageResources($resourceIds: [ID!]!, $locale: String!) {
...
const Index = () => {
  ...
  const renderTranslateAction = (record: any, resourceType: string) => {
    ...
    return (
      <SingleTranslateAction
        ...
        isOutdated={isManageTranslationOutdated(pagesData, record?.resourceId, record?.shopifyKey)}
  ...
  const SEOColumns = [
    {
      title: "SEO",
      key: "resource",
      render: (_: any, record: any) => renderManageField(record, "PAGE"),
    },
  ];
  ...
  const refreshCurrentPageData = () => {
    ...
    dataFetcher.submit(
      ...
      {
        method: "post",
        action: `/app/manage_translation/page?language=${selectedLanguage}`,
      },
    );
  };
  const onNext = () => {
            ...
            >
              {isMobile ? (
                <Space direction="vertical" style={{ width: "100%" }}>
                  ...
                  {renderMobileSection(t("Resource"), resourceData, "PAGE", {
                    isHtml: (record) => record?.shopifyKey == "body_html",
                  })}
                  {renderMobileSection(t("SEO"), SeoData, "PAGE")}
                  <SideMenu
                    items={menuData}
                    selectedKeys={selectPageKey}
                    onClick={handleMenuChange}
