...
const Index = () => {
  ...
  useEffect(() => {
    ...
    fetcher.submit(
      {
        log: `${globalStore?.shop} 目前在翻译管�?电子邮件页面`,
      },
      {
        method: "POST",
        action: "/log",
  ...
  const renderTranslateAction = (record: any) => {
    ...
    return (
      <SingleTranslateAction
        ...
        isOutdated={isManageTranslationOutdated(emailsData, record?.resourceId, record?.shopifyKey)}
  ...
  const handleTranslate = async ({
    ...
    fetcher.submit(
      {
        log: `${globalStore?.shop} 从翻译管�?电子邮件页面点击单行翻译`,
      },
      {
        method: "POST",
        action: "/log",
    ...
    if (data?.success) {
      if (loadingItemsRef.current.includes(record?.key)) {
        ...
        fetcher.submit(
          {
            log: `${globalStore?.shop} 从翻译管�?电子邮件页面点击单行翻译返回结果 ${data?.response}`,
          },
          {
            method: "POST",
            action: "/log",
  ...
  const refreshCurrentPageData = () => {
    const currentResourceIds = emailsData
    ...
    dataFetcher.submit(
      ...
      {
        method: "post",
        action: `/app/manage_translation/email?language=${selectedLanguage}`,
      },
    );
  };
  const onNext = () => {
