    ...
    case refreshResourceIds.length > 0:
      try {
        const response = await admin.graphql(
          `#graphql
            query refreshShippingResources($resourceIds: [ID!]!, $locale: String!) {
...
const Index = () => {
  ...
  useEffect(() => {
    ...
    fetcher.submit(
      {
        log: `${globalStore?.shop} 目前在翻译管�?配送方式页面`,
      },
      {
        method: "POST",
        action: "/log",
  ...
  const handleTranslate = async ({
    ...
    fetcher.submit(
      {
        log: `${globalStore?.shop} 从翻译管�?配送方式页面点击单行翻译`,
      },
      {
        method: "POST",
        action: "/log",
    ...
    if (data?.success) {
      if (loadingItemsRef.current.includes(record?.key)) {
        ...
        fetcher.submit(
          {
            log: `${globalStore?.shop} 从翻译管�?配送方式页面点击单行翻译返回结�?${data?.response}`,
          },
          {
            method: "POST",
            action: "/log",
  ...
  const refreshCurrentPageData = () => {
    const currentResourceIds = shippingsData
    ...
    dataFetcher.submit(
      ...
      {
        method: "post",
        action: `/app/manage_translation/shipping?language=${selectedLanguage}`,
      },
    );
  };
