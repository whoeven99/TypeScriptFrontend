  ...
  if (refreshResourceIds.length > 0) {
    try {
      const response = await admin.graphql(
          `#graphql
            query refreshBlogResources($resourceIds: [ID!]!, $locale: String!) {
...
const Index = () => {
  ...
  useEffect(() => {
    ...
    fetcher.submit(
      {
        log: `${globalStore?.shop} 目前在翻译管�?博客页面`,
      },
      {
        method: "POST",
        action: "/log",
  ...
  const renderTranslateAction = (record: any) => {
    ...
    return (
      <SingleTranslateAction
        ...
        isOutdated={isManageTranslationOutdated(blogsData, record?.resourceId, record?.shopifyKey)}
  ...
  const handleTranslate = async ({
    ...
    fetcher.submit(
      {
        log: `${globalStore?.shop} 从翻译管�?博客页面点击单行翻译`,
      },
      {
        method: "POST",
        action: "/log",
    ...
    if (data?.success) {
      if (loadingItemsRef.current.includes(record?.key)) {
        ...
        fetcher.submit(
          {
            log: `${globalStore?.shop} 从翻译管�?博客页面点击单行翻译返回结果 ${data?.response}`,
          },
          {
            method: "POST",
            action: "/log",
  ...
  const refreshCurrentPageData = () => {
    const currentResourceIds = blogsData
    ...
    dataFetcher.submit(
      ...
      {
        method: "post",
        action: `/app/manage_translation/blog?language=${selectedLanguage}`,
      },
    );
  };
  const onNext = () => {
