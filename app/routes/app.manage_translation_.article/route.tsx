...
export const action = async ({ request }: ActionFunctionArgs) => {
  ...
  if (refreshResourceIds.length > 0) {
    try {
      ...
      logManageTranslationGraphQLErrorDetail(
        "Error refreshing current article page",
        error,
      );
      return buildManageActionErrorResponse(error, { response: undefined });
    }
  ...
  const renderTranslateAction = (record: any, resourceType: string) => {
    ...
    return (
      <SingleTranslateAction
        ...
        isOutdated={isManageTranslationOutdated(articlesData, record?.resourceId, record?.shopifyKey)}
  ...
  const resourceColumns = [
    {
      ...
      render: (_: any, record: any) =>
        renderManageField(record, "ARTICLE", {
          isHtml:
  ...
  const SEOColumns = [
    {
      title: "SEO",
      key: "resource",
      render: (_: any, record: any) => renderManageField(record, "ARTICLE"),
    },
  ];
