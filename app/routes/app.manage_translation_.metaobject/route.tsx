    ...
    case refreshResourceIds.length > 0:
      try {
        const response = await admin.graphql(
          `#graphql
            query refreshMetaobjectResources($resourceIds: [ID!]!, $locale: String!) {
...
const Index = () => {
  ...
  useEffect(() => {
    ...
    fetcher.submit(
      {
        log: `${globalStore?.shop} 目前在翻译管�?元对象页面`,
      },
      {
        method: "POST",
        action: "/log",
    ...
    if (confirmFetcher.data?.success) {
      ...
      if (Array.isArray(successfulItem) && successfulItem.length) {
        successfulItem.forEach((item: any) => {
          const index = metaobjectsData.findIndex(
            (option: any) => option.resourceId === item?.response?.resourceId,
          );
          if (index !== -1) {
            const data = metaobjectsData[index]?.translations?.find(
              (option: any) => option?.key === item?.response?.key,
            );
            if (data) {
              data.value = item?.response?.value;
            } else {
              metaobjectsData[index].translations.push({
  ...
  const generateMenuItemsArray = (items: any) => {
    return items.flatMap((item: any, index: number) => {
      if (item?.translatableContent.length !== 0) {
        // 创建当前项的对象
        const currentItem = {
          key: `${item?.translatableContent[0]?.key}_${item?.resourceId}_${index}`,
          resourceId: item?.resourceId,
          shopifyKey: item?.translatableContent[0]?.key,
          index,
          resource: t("name"),
  ...
  const handleTranslate = async ({
    ...
    fetcher.submit(
      {
        log: `${globalStore?.shop} 从翻译管�?元对象页面点击单行翻译`,
      },
      {
        method: "POST",
        action: "/log",
    ...
    if (data?.success) {
      if (loadingItemsRef.current.includes(record?.key)) {
        ...
        fetcher.submit(
          {
            log: `${globalStore?.shop} 从翻译管�?元对象页面点击单行翻译返回结�?${data?.response}`,
          },
          {
            method: "POST",
            action: "/log",
  ...
  const refreshCurrentPageData = () => {
    const currentResourceIds = metaobjectsData
    ...
    dataFetcher.submit(
      ...
      {
        method: "post",
        action: `/app/manage_translation/metaobject?language=${selectedLanguage}`,
      },
    );
  };
  const onNext = () => {
