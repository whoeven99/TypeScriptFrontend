...
const Index = () => {
  ...
  const { shop, searchTerm, collections, server, shopName } =
  ...
  const handleTranslate = async (
    ...
    if (data?.success) {
      if (loadingItemsRef.current.includes(key)) {
        ...
        fetcher.submit(
          {
            log: `${shop} 从翻译管理-集合页面点击单行翻译返回结果 ${data?.response}`,
          },
          {
            method: "POST",
            action: "/log",
