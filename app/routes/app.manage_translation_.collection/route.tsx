...
const Index = () => {
  ...
  useEffect(() => {
    if (confirmData.length > 0) {
      shopify.saveBar.show("collection-confirm-save");
    } else {
      shopify.saveBar.hide("collection-confirm-save");
    }
  }, [confirmData]);
  ...
  return (
    <>
      <SaveBar id="collection-confirm-save">
        <button
          variant="primary"
                  ...
                  <Space direction="vertical" style={{ width: '100%' }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                      <Title level={4} style={{ margin: 0, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                        {menuData!.find((item: any) => item.key === selectCollectionKey)?.label}
                      </Title>
