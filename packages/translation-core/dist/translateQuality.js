...
export function looksLikeUntranslated(source, translated, target) {
...
export function looksLikeWrongScriptLeak(source, translated, target) {
    const tl = targetLangCode(target);
    if (["zh", "ja", "ko"].includes(tl))
        return false;
    return hasCjk(translated) && !hasCjk(source);
}
/** LLM invented content for an empty source leaf (e.g. empty description → "S3"). */
export function looksLikeEmptySourceHallucination(source, translated) {
    if (source.trim() !== "")
        return false;
    return translated.trim() !== "";
}
/** Model echoed the prompt's "number" wording instead of preserving ⟦N⟧ sentinels. */
export function hasPromptSentinelLeakage(text) {
    return /\[number\]/i.test(text);
}
/** Glossary target must not inject CJK into non-CJK locales unless source already has CJK. */
export function glossaryTargetMatchesLocale(targetText, sourceText, target) {
    const tl = targetLangCode(target);
    if (["zh", "ja", "ko"].includes(tl))
        return true;
    if (hasCjk(targetText) && !hasCjk(sourceText))
        return false;
    return true;
}
export function isTranslatableLeafText(text) {
    return text.trim().length > 0;
}
//# sourceMappingURL=translateQuality.js.map