...
import { applyJsonSlotTranslations, extractJsonTextSlots, isListFormat, shouldTranslateMetafieldJson, tryParseJsonContainer, } from "./jsonExtractRules.js";
import { isHtmlContent } from "./htmlContent.js";
import { hasPromptSentinelLeakage, isTranslatableLeafText, looksLikeEmptySourceHallucination, looksLikeUntranslated, looksLikeWrongScriptLeak, } from "./translateQuality.js";
import { effectiveTranslation, hasHtmlPlaceholderLeak, htmlNodePartsOf, restoreBrPlaceholders, restoreHtmlTextNodes, roundtripHtmlForTest, sanitizeHtmlTextTranslation, } from "./htmlTranslate.js";
import { enforceTranslateResultLimits } from "./translationFieldLimits.js";
import { maskPlaceholders, placeholdersIntact, protectedLiteralsPreserved, restoreMaskedPlaceholders, } from "./placeholderMask.js";
...
class AdaptiveSemaphore {
    ...
    _inflight = 0;
    _waiters = [];
    constructor(initial) { this._max = Math.max(1, initial); }
    setMax(n) {
...
class EWMA {
    _v;
    _a;
    constructor(_v, _a = 0.2) {
        this._v = _v;
        this._a = _a;
    }
    update(sample) { this._v = this._a * sample + (1 - this._a) * this._v; }
...
function formatLimitHintValue(value) {
    if (value == null)
        return String(value);
...
function collectLimitHints(value, path = "", out = [], depth = 0) {
    if (depth > 8 || out.length >= LIMIT_HINT_MAX)
        return out;
    if (value == null)
        return out;
...
function formatLimitHintsForLog(hints) {
    if (hints.length === 0)
        return "";
    return `\n  limit-related in response body:\n${hints.map((h) => `    ${h}`).join("\n")}`;
}
function limitLikeHeaderLines(headers) {
 ...
 */
function parseRateLimitResetMs(raw, now) {
...
const RAMP_LATENCY_INHIBIT_MS = Math.max(10_000, Number(process.env.TRANSLATE_RAMP_LATENCY_INHIBIT_MS) ||
...
const TIMEOUT_RATE_HALF_LIFE_MS = Math.max(5_000, Number(process.env.TRANSLATE_TIMEOUT_RATE_HALF_LIFE_MS) || 30_000);
/** If no timeout for this long, allow timed concurrency recovery. */
const RECOVERY_NO_TIMEOUT_MS = Math.max(2_000, Number(process.env.TRANSLATE_RECOVERY_NO_TIMEOUT_MS) || 8_000);
/** Min interval between timed recovery ticks. */
const RECOVERY_RAMP_INTERVAL_MS = Math.max(3_000, Number(process.env.TRANSLATE_RECOVERY_RAMP_INTERVAL_MS) || 10_000);
...
export function sanitizeDeepSeekUserId(shop) {
...
export function resolveDeepSeekAccountConcurrencyLimit(model) {
...
export function resolveDeepSeekPoolConcurrency(model) {
    const accountLimit = resolveDeepSeekAccountConcurrencyLimit(model);
    const util = Math.min(1, Math.max(0.1, Number(process.env.DEEPSEEK_CONCURRENCY_UTIL) || 0.45));
    const ceiling = Math.min(MAX_POOL_CONCURRENCY, Math.max(1, Math.floor(accountLimit * util)));
...
function classifyLlmError(e) {
    ...
    if (/^DeepSeek HTTP \d/.test(msg))
        return "http";
    if (/empty response body/i.test(msg))
        return "http";
    if (/DeepSeek API error/i.test(msg))
        return "api";
    if (e instanceof SyntaxError || /json|unexpected token|no json object/i.test(msg))
        return "parse";
    return "other";
}
/** Map DEEPSEEK_BASE_URL → POST .../chat/completions (DeepSeek native endpoint). */
function resolveDeepSeekChatCompletionsUrl(baseURL) {
...
function extractJsonStringField(raw, field) {
    ...
    while (i < raw.length && (raw[i] === " " || raw[i] === "\t"))
        i++;
    if (raw[i] !== '"')
        return null; // null / number / object → not a plain string
...
async function fetchDeepSeekChatCompletion(apiKey, chatUrl, model, messages, timeoutMs, firstTokenTimeoutMs, userId) {
    ...
    const armIdle = () => {
        ...
        idleTimer = setTimeout(() => controller.abort(new LlmTimeoutError(gotContent ? "idle" : "first-token")), gotContent ? LLM_IDLE_TIMEOUT_MS : firstTokenTimeoutMs);
    };
    const hardTimer = setTimeout(() => controller.abort(new LlmTimeoutError("hard")), timeoutMs);
    armIdle();
    try {
        ...
        const handleLine = (line) => {
            ...
            if (data.includes('"total_tokens"') ||
                data.includes('"usage"') ||
                data.includes('"error"')) {
                try {
                    const evt = JSON.parse(data);
                    if (evt.error?.message)
                        apiErrorMsg = evt.error.message;
                    if (evt.usage?.total_tokens)
                        tokens = evt.usage.total_tokens;
                }
                catch {
        ...
        if (buffer.length > scanFrom)
            handleLine(buffer.slice(scanFrom).trim());
        if (apiErrorMsg)
            throw new Error(`DeepSeek API error: ${apiErrorMsg}`);
        return {
            content: content || "{}",
            tokens,
            response: resp,
...
async function invokeChatCompletion(transport, model, messages, timeoutMs, firstTokenTimeoutMs, deepseekUserId) {
    return fetchDeepSeekChatCompletion(transport.apiKey, transport.chatUrl, model, messages, timeoutMs, firstTokenTimeoutMs, deepseekUserId);
}
// ── GPT / Azure OpenAI 引擎 ───────────────────────────────────────────────────
...
const GPT_ENDPOINT = (process.env.Gpt_Endpoint?.trim() || "https://eastus.api.cognitive.microsoft.com").replace(/\/+$/, "");
...
let _gptInFlight = 0;
const _gptWaiters = [];
async function gptAcquire() {
    if (_gptInFlight < GPT_CONCURRENCY) {
        _gptInFlight++;
        return;
    }
    await new Promise((resolve) => _gptWaiters.push(resolve));
    _gptInFlight++;
}
function gptRelease() {
    _gptInFlight--;
...
async function callAzureOpenAIChat(model, messages, itemCount) {
    const key = gptApiKey();
    if (!key)
        throw new Error("Gpt_ApiKey 未配置");
    const url = `${GPT_ENDPOINT}/openai/deployments/${encodeURIComponent(model)}/chat/completions?api-version=${GPT_API_VERSION}`;
    await gptAcquire();
    try {
        for (let attempt = 0; attempt < 3; attempt++) {
            const controller = new AbortController();
            const timer = setTimeout(() => controller.abort(new LlmTimeoutError("hard")), llmTimeoutMsForBatch(itemCount));
            try {
...
const KIMI_BASE_URL = (process.env.KIMI_BASE_URL?.trim() || "https://api.moonshot.cn/v1").replace(/\/+$/, "");
...
export function isKimiModel(aiModel) {
    return kimiConfigured() && /^kimi/i.test((aiModel ?? "").trim());
}
function resolveKimiModel(aiModel) {
    const m = (aiModel ?? "").trim();
    return /^kimi/i.test(m) ? m : KIMI_DEFAULT_MODEL;
}
let _kimiInFlight = 0;
const _kimiWaiters = [];
async function kimiAcquire() {
    if (_kimiInFlight < KIMI_CONCURRENCY) {
        _kimiInFlight++;
        return;
    }
    await new Promise((resolve) => _kimiWaiters.push(resolve));
    _kimiInFlight++;
}
function kimiRelease() {
    _kimiInFlight--;
...
async function callKimiChat(model, messages, itemCount) {
...
const GEMINI_API_BASE = (process.env.GEMINI_API_BASE?.trim() ||
...
export function isGeminiModel(aiModel) {
    return geminiConfigured() && /^gemini/i.test((aiModel ?? "").trim());
}
function resolveGeminiModel(aiModel) {
    const m = (aiModel ?? "").trim();
    return /^gemini/i.test(m) ? m : GEMINI_DEFAULT_MODEL;
}
let _geminiInFlight = 0;
const _geminiWaiters = [];
async function geminiAcquire() {
    if (_geminiInFlight < GEMINI_CONCURRENCY) {
        _geminiInFlight++;
        return;
    }
    await new Promise((resolve) => _geminiWaiters.push(resolve));
    _geminiInFlight++;
}
...
function geminiMessagesToParts(messages) {
    const systemParts = [];
    const userParts = [];
...
async function callGeminiChat(model, messages, itemCount) {
...
class LLMKeyPool {
    ...
    constructor(slots, options) {
        ...
        console.log(`[llm-pool] initialised — ${slots.length} slot(s): ${slotLabels}, ` +
        ...
        for (let i = 0; i < this.slots.length; i++) {
            ...
            if (slot.throttledUntil <= now) {
                ...
                return {
                    ...
                    model: slot.model,
                    label: slot.label,
                    onResponse: (headers, durationMs, tokens, limitHints = []) => {
                        const headersApplied = this._applyHeaders(slot, headers);
                        if (headersApplied)
                            this._hasSeenAnyHeaders = true;
                        this.latency.update(durationMs);
                        if (tokens > 0)
                            this.tokPerReq.update(tokens);
                        slot.stats.calls++;
    ...
    _applyHeaders(slot, h) {
        ...
        if (limitReq == null || remReq == null || resetReqS == null)
    ...
    _logResponseQuota(slot, allHeaders, headersApplied, durationMs, tokens, limitHints = []) {
        ...
        if (!this._firstResponseLogged.has(slot.label)) {
            ...
            if (headersApplied && slot.rateLimit) {
                ...
                console.log(`[llm-pool] ${slot.label} first response — ${formatSlotQuota(slot.rateLimit)}` +
                ...
                console.log(`[llm-pool] ${slot.label} first response — deepseek concurrency mode` +
                ...
                console.log(`[llm-pool] ${slot.label} first response — no recognized rate-limit headers` +
                    ` (${durationMs.toFixed(0)}ms, ${tokens} tok)` +
                    (headerRlLines
    ...
    _maybeLogQuota(slot, reason) {
        ...
        const lowQuota = rl.limitReq > 0 && rl.remainingReq / rl.limitReq < 0.2 ||
        ...
        if (!changed && !lowQuota && !intervalElapsed)
            return;
        this._quotaLogAt.set(slot.label, now);
        this._lastQuotaSnap.set(slot.label, snap);
        console.log(`[llm-pool] ${slot.label} quota ${reason} — ${snap}, pool concurrency=${this.sem.max}` +
    ...
    _blindOnSuccess() {
        ...
        if (this._blindSuccesses % RAMP_STEP === 0 && this.sem.max < blindCeil) {
            ...
            console.log(`[llm-pool] blind ramp → concurrency=${newMax}/${blindCeil}` +
    ...
    _deepseekOnSuccess() {
        ...
        if (this._timeoutRate.value > LLM_TIMEOUT_RATE_HIGH / 2)
            return;
        if (this.latency.value > RAMP_LATENCY_INHIBIT_MS)
            return;
        ...
        let rampAdd;
        if (lat < 3000)
            rampAdd = 8; // fast climb — plenty of headroom
        else if (lat < 6000)
            rampAdd = 4; // normal
        else if (lat < 10000)
            rampAdd = 2; // slow — approaching the knee
        else if (lat < 15000)
            rampAdd = 1; // crawl — near capacity
        else
            rampAdd = 0; // hold — already at inhibit threshold
        if (rampAdd === 0)
            return;
        ...
        if (this._deepseekRampSuccesses % rampStep === 0 &&
            ...
            if (newMax !== this.sem.max) {
                this.sem.setMax(newMax);
                console.log(`[llm-pool] deepseek ramp → concurrency=${newMax}/${this._deepseekConcCeiling}` +
     ...
     */
    _applyTimeoutRateTimeDecay(now = Date.now()) {
     ...
     */
    _maybeTimedRecovery(now = Date.now()) {
        if (this.sem.max >= this._deepseekConcCeiling)
            return;
        if (now - this._lastTimedRecoveryAt < RECOVERY_RAMP_INTERVAL_MS)
            return;
        if (this._lastTimeoutAt > 0 && now - this._lastTimeoutAt < RECOVERY_NO_TIMEOUT_MS)
            return;
        if (this._timeoutRate.value > LLM_TIMEOUT_RATE_HIGH / 2)
            return;
        ...
        if (newMax === this.sem.max)
            return;
        this.sem.setMax(newMax);
        const quietSec = this._lastTimeoutAt > 0 ? Math.round((now - this._lastTimeoutAt) / 1000) : null;
        console.log(`[llm-pool] timed recovery → concurrency=${newMax}/${this._deepseekConcCeiling}` +
    ...
    _softBackoff(reason) {
        const now = Date.now();
        if (now - this._lastSoftBackoffAt < SOFT_BACKOFF_MIN_INTERVAL_MS)
            return;
        ...
        if (newMax !== this.sem.max) {
            this.sem.setMax(newMax);
            console.warn(`[llm-pool] soft back-off → concurrency=${newMax} (${reason}; ` +
    ...
    firstTokenBudgetMs() {
        ...
        return Math.min(LLM_FIRST_TOKEN_TIMEOUT_MAX_MS, Math.max(LLM_FIRST_TOKEN_TIMEOUT_MS, Math.round(fromLatency)));
    }
    /** Record fields that exhausted retries and fell back to the original text. */
    recordTerminalFallback(n = 1) {
    ...
    _recalc() {
        ...
        if (newMax !== this.sem.max) {
            ...
            console.log(`[llm-pool] concurrency ${this.sem.max} → ${newMax}` +
        ...
        for (const slot of this.slots) {
            if (slot.throttledUntil > now || !slot.rateLimit)
                continue;
...
export function resetLlmPoolForTests() {
    _pool = null;
}
...
export function getLlmErrorBreakdown() {
...
export function recordLlmTerminalFallback(n = 1) {
    _pool?.recordTerminalFallback(n);
}
export async function flushKeyStats() {
    const now = Date.now();
    if (now - _lastStatFlush < STAT_FLUSH_INTERVAL_MS)
        return;
    _lastStatFlush = now;
    if (!_pool)
        return;
...
const _shopQuotaGates = new Map();
function getShopQuotaGate(shop) {
...
export function setShopQuotaCap(shop, cap) {
...
function llmAvailableFor(aiModel) {
    const m = (aiModel ?? "").trim();
    if (/^gpt[-.]/i.test(m))
        return gptConfigured();
    if (/^kimi/i.test(m))
...
function forcedEngine(aiModel) {
    if (aiModel?.trim().toLowerCase() === "google-translate")
        return "google";
    return null;
}
// Plain fields at or above this length are treated as "rich" content.
const SHORT_PLAIN_THRESHOLD = 80;
function fieldTier(key, value, klass) {
    if (isHandleFieldKey(key))
        return "rich";
    if (klass === "html" || klass === "json" || klass === "list")
        return "rich";
    if (key === "meta_description")
        return "rich";
    return value.length >= SHORT_PLAIN_THRESHOLD ? "rich" : "trivial";
}
function poolSignature(order, isHandle) {
    const base = order.join(",");
    return isHandle ? `${HANDLE_POOL_PREFIX}${base}` : base;
}
function parsePoolSignature(sig) {
    if (sig.startsWith(HANDLE_POOL_PREFIX)) {
        return {
            isHandle: true,
            order: sig.slice(HANDLE_POOL_PREFIX.length).split(","),
        };
    }
    return { isHandle: false, order: sig.split(",") };
}
/** Ordered engine candidates for a tier (primary first, then fallback). */
function engineOrderFor(tier, aiModel) {
    ...
    if (order.length === 0)
        order.push(l ? "llm" : "google");
    return order;
}
/** The model/label recorded for a chosen engine (used for TM cache + Cosmos). */
function engineModel(engine, aiModel) {
    if (engine === "google")
        return "google-translate";
    if (isGptModel(aiModel))
        return resolveGptModel(aiModel);
    if (isKimiModel(aiModel))
        return resolveKimiModel(aiModel);
    if (isGeminiModel(aiModel))
        return resolveGeminiModel(aiModel);
 ...
 */
export function resolveEngine(aiModel) {
    const forced = forcedEngine(aiModel);
    if (forced === "google")
        return { provider: "google", model: "google-translate" };
    if (isGptModel(aiModel))
        return { provider: "azure-openai", model: resolveGptModel(aiModel) };
    if (isKimiModel(aiModel))
        return { provider: "kimi", model: resolveKimiModel(aiModel) };
    if (isGeminiModel(aiModel))
        return { provider: "gemini", model: resolveGeminiModel(aiModel) };
...
function meetsScriptThreshold(text, ...patterns) {
    let count = 0;
    for (const re of patterns) {
        count += countRegexMatches(text, re);
    }
    if (count < TARGET_SCRIPT_MIN_CHARS)
        return false;
...
function hasAnyScriptMatch(text, ...patterns) {
    return patterns.some((re) => re.test(text));
}
function scriptPatternsForLang(lang) {
    switch (lang) {
        ...
        case "ru":
        case "uk":
        case "bg": return [CYRILLIC_RE];
        case "th": return [THAI_RE];
        case "hi":
        case "mr":
        case "ne": return [DEVANAGARI_RE];
        case "pl": return [POLISH_DIACRITIC_RE];
        case "de": return [GERMAN_DIACRITIC_RE];
        case "fr": return [FRENCH_DIACRITIC_RE];
        case "es":
        case "pt": return [IBERIAN_DIACRITIC_RE];
        case "cs":
        case "sk": return [CZECH_SLOVAK_DIACRITIC_RE];
...
function isCrossScriptFamilyPair(source, target, langs) {
...
export function alreadyInTarget(text, source, target) {
    ...
    if (isCrossScriptFamilyPair(source, target, ["ru", "uk", "bg"]))
        return false;
    if (isCrossScriptFamilyPair(source, target, ["hi", "mr", "ne"]))
        return false;
    if (isCrossScriptFamilyPair(source, target, ["cs", "sk"]))
        return false;
    if (isCrossScriptFamilyPair(source, target, ["es", "pt"]))
        return false;
 ...
 */
export function containsSourceScript(text, source) {
...
export function classifyField(key, value, shopifyType) {
    if (value !== undefined) {
...
function countJsonRuleUnits(value) {
 ...
 */
export function countFieldUnits(key, value, shopifyType) {
...
const MAX_CHARS_PER_BATCH = Math.max(500, Number(process.env.TRANSLATE_MAX_CHARS_PER_BATCH) || 3_000);
/** Max items per LLM request — avoids 80+ key payloads that routinely hit the timeout. */
const MAX_ITEMS_PER_BATCH = Math.max(1, Number(process.env.TRANSLATE_MAX_ITEMS_PER_BATCH) || 25);
/** Rich (HTML/JSON/LLM-first) pools use smaller batches — fewer keys per request, less idle-timeout tail. */
const RICH_MAX_CHARS_PER_BATCH = Math.max(500, Number(process.env.TRANSLATE_RICH_MAX_CHARS_PER_BATCH) || 1_500);
const RICH_MAX_ITEMS_PER_BATCH = Math.max(1, Number(process.env.TRANSLATE_RICH_MAX_ITEMS_PER_BATCH) || 8);
/** LLM-first engine order ⇒ rich tier (HTML/JSON/long plain). Google-first ⇒ trivial. */
export function resolveBatchLimits(order) {
    if (order[0] === "llm") {
...
const LLM_TIMEOUT_BASE_MS = Math.max(60_000, Number(process.env.TRANSLATE_LLM_TIMEOUT_MS) || 300_000);
const LLM_TIMEOUT_PER_ITEM_MS = Math.max(500, Number(process.env.TRANSLATE_LLM_TIMEOUT_PER_ITEM_MS) || 5_000);
const LLM_TIMEOUT_MAX_MS = Math.max(LLM_TIMEOUT_BASE_MS, Number(process.env.TRANSLATE_LLM_TIMEOUT_MAX_MS) || 600_000);
/** Streaming idle: no token for this long after generation started → abort. */
const LLM_IDLE_TIMEOUT_MS = Math.max(10_000, Number(process.env.TRANSLATE_LLM_IDLE_TIMEOUT_MS) || 300_000);
/**
 * 「等首个 token」窗口下限；实际上限由 `firstTokenBudgetMs()` 自适应，不超过
 * `LLM_FIRST_TOKEN_TIMEOUT_MAX_MS`。默认与 idle 对齐，避免排队中被过早砍掉。
 */
const LLM_FIRST_TOKEN_TIMEOUT_MS = Math.max(LLM_IDLE_TIMEOUT_MS, Number(process.env.TRANSLATE_LLM_FIRST_TOKEN_TIMEOUT_MS) || 180_000);
/** Multiplier on observed avg latency for the adaptive first-token budget. */
const LLM_FIRST_TOKEN_LATENCY_FACTOR = Math.max(1, Number(process.env.TRANSLATE_LLM_FIRST_TOKEN_LATENCY_FACTOR) || 6);
/** Hard ceiling on the adaptive first-token wait (ms). */
const LLM_FIRST_TOKEN_TIMEOUT_MAX_MS = Math.max(LLM_FIRST_TOKEN_TIMEOUT_MS, Number(process.env.TRANSLATE_LLM_FIRST_TOKEN_TIMEOUT_MAX_MS) || 300_000);
/** On timeout, re-chunk a large batch straight to this size (skip the slow cascade). */
const TIMEOUT_RESPLIT_SIZE = Math.max(1, Number(process.env.TRANSLATE_TIMEOUT_RESPLIT_SIZE) || 3);
/** First-token timeouts: same-batch retries before re-chunking (queue drain). */
const FIRST_TOKEN_DRAIN_RETRIES = Math.max(0, Number(process.env.TRANSLATE_FIRST_TOKEN_DRAIN_RETRIES) || 3);
/** Drain delay before a first-token same-batch retry (lets the server queue clear). */
const FIRST_TOKEN_DRAIN_MS = Math.max(0, Number(process.env.TRANSLATE_FIRST_TOKEN_DRAIN_MS) || 5_000);
/** Base backoff between single-item leaf retries (grows linearly per attempt). */
const LEAF_RETRY_BACKOFF_MS = Math.max(0, Number(process.env.TRANSLATE_LEAF_RETRY_BACKOFF_MS) || 2_000);
/** Scale timeout with batch size so large (but capped) batches get more wall clock. */
export function llmTimeoutMsForBatch(itemCount) {
    const n = Math.max(1, itemCount);
    return Math.min(LLM_TIMEOUT_MAX_MS, LLM_TIMEOUT_BASE_MS + Math.max(0, n - 1) * LLM_TIMEOUT_PER_ITEM_MS);
}
 ...
 */
export async function pAll(items, concurrency, fn) {
    if (items.length === 0)
        return [];
    if (concurrency <= 1)
...
function batchByChars(items, maxChars, maxItems = MAX_ITEMS_PER_BATCH) {
    const batches = [];
    let current = [];
    let currentChars = 0;
...
async function callGoogleTranslate(texts, target, format) {
    const apiKey = process.env.GOOGLE_TRANSLATE_API_KEY?.trim();
    if (!apiKey)
        throw new Error("GOOGLE_TRANSLATE_API_KEY is required");
    const resp = await fetch(`https://translation.googleapis.com/language/translate/v2?key=${encodeURIComponent(apiKey)}`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
    ...
    if (!resp.ok)
        throw new Error(`Google Translate HTTP ${resp.status}: ${await resp.text()}`);
    const data = (await resp.json());
    return data.data.translations.map((t) => t.translatedText);
}
async function translateItemsRouted(items, source, target, aiModel, shopName, order, promptKind = "default", customPrompt = "", 
...
logSingleTranslate = false) {
    ...
    const collected = new Map(); // masked translations
    const engineByKey = new Map(); // which engine resolved each key
    const llmTokensByKey = new Map(); // LLM API tokens charged per key
    ...
    for (const engine of order) {
        ...
        if (engine === "llm") {
            ...
            try {
                await gatherTranslations(missing, aiModel, systemPrompt, collected, tokenAccum, shopName, FIRST_TOKEN_DRAIN_RETRIES, logSingleTranslate);
            }
            catch (e) {
            ...
            for (const batch of batchByChars(missing, MAX_CHARS_PER_BATCH)) {
                try {
                    ...
                    if (googleMap.size > 0) {
                        logLlmTranslateOutcome("google-translate", shopName, batch.filter((b) => googleMap.has(b.key)), googleMap, 0);
                    }
                }
                catch (e) {
    ...
    for (const it of items) {
        ...
        if (placeholders.length > 0) {
            const tokensOk = (placeholders.every((t) => restored.includes(t)) ||
...
async function retryPoolFallbacks(translated, pools, source, target, aiModel, shopName, shouldAbort, customPrompt = "", onLeafTranslated, logSingleTranslate = false) {
    let retried = 0;
    for (const [sig, occ] of pools) {
        ...
        for (const text of needsRetry) {
            ...
            const { results: m } = await translateItemsRouted([{ key: "0", value: text, digest: "" }], source, target, aiModel, shopName, poolOrder, isHandle ? "handle" : "default", customPrompt, logSingleTranslate);
            const r = m.get("0");
            if (r?.status === "translated" && !looksLikeUntranslated(text, r.value, target) && !looksLikeWrongScriptLeak(text, r.value, target)) {
                tmap.set(text, r);
                retried++;
                if (onLeafTranslated)
                    onLeafTranslated(text, r, poolPrimaryModel);
            }
        }
    }
    return retried;
...
const LEAF_RETRIES = Math.max(1, Number(process.env.TRANSLATE_LEAF_RETRIES) || 5);
/**
 ...
 */
function decodeQuoteEntities(text) {
    return text
        .replace(/&#0*39;|&apos;/g, "'")
        .replace(/&#0*34;|&quot;/g, '"');
}
...
function sanitizeJsonSlotTranslation(original, translated) {
    const t = sanitizeHtmlTextTranslation(original, translated);
    if ((/"type"\s*:|"children"\s*:|]\s*,\s*"type"/.test(t)) &&
 ...
 */
function buildSystemPrompt(target, glossaryLines, profileBlock = "", userInstruction = "") {
...
function buildHandleSystemPrompt(target, glossaryLines, profileBlock = "", userInstruction = "") {
    const glossaryBlock = glossaryLines.length
        ? `\nGlossary (apply consistently):\n${glossaryLines.join("\n")}\n`
        : "";
    const shopContextBlock = profileBlock ? `\n${profileBlock}\n` : "";
...
function parseTranslationResult(raw, tokens, idToKey) {
    ...
    for (const r of parsed) {
        if (typeof r?.key === "string" && typeof r?.translatedValue === "string") {
            ...
            if (origKey !== undefined)
                map.set(origKey, r.translatedValue);
        }
    }
    return { map, tokens };
}
...
function truncateForTranslateLog(value, max = LLM_TRANSLATE_LOG_VALUE_MAX) {
    if (value.length <= max)
        return value;
 ...
 */
function logLlmTranslateOutcome(model, shopName, items, map, tokens) {
...
async function callLLMOnce(items, aiModel, systemPrompt, shopName, logSingleTranslate = false) {
    ...
    const logLlmReturn = (model, raw, tokens) => {
        if (!logSingleTranslate)
            return;
    ...
    const finishOk = (model, raw, tokens) => {
    ...
    if (isGptModel(aiModel)) {
        try {
            ...
            const { raw, tokens } = await callAzureOpenAIChat(model, messages, items.length);
    ...
    const t0 = Date.now();
    try {
        const deepseekUserId = acq.transport.kind === "deepseek-fetch" && shopName
            ? sanitizeDeepSeekUserId(shopName)
            : undefined;
        const { content: raw, tokens, response, limitHints } = await invokeChatCompletion(acq.transport, model, messages, llmTimeoutMsForBatch(items.length), getPool().firstTokenBudgetMs(), deepseekUserId);
        ...
        if (quotaGate)
            quotaGate.release(); // release per-shop quota gate
    }
}
/**
 ...
 */
async function gatherTranslations(items, aiModel, systemPrompt, collected, tokenAccum, shopName, firstTokenRetriesLeft = FIRST_TOKEN_DRAIN_RETRIES, logSingleTranslate = false) {
    ...
    if (pend.length > MAX_ITEMS_PER_BATCH) {
        ...
        console.log(`[llm] batch of ${pend.length} items exceeds cap ${MAX_ITEMS_PER_BATCH}; splitting proactively`);
        await gatherTranslations(pend.slice(0, mid), aiModel, systemPrompt, collected, tokenAccum, shopName, FIRST_TOKEN_DRAIN_RETRIES, logSingleTranslate);
        await gatherTranslations(pend.slice(mid), aiModel, systemPrompt, collected, tokenAccum, shopName, FIRST_TOKEN_DRAIN_RETRIES, logSingleTranslate);
        return;
    }
    try {
        const { map, tokens } = await callLLMOnce(pend, aiModel, systemPrompt, shopName, logSingleTranslate);
        ...
        if (pend.length > 1) {
            ...
            if (timeoutKind === "first-token" && firstTokenRetriesLeft > 0) {
                console.warn(`[llm] batch of ${pend.length} timed out waiting for first token; ` +
                ...
                await gatherTranslations(pend, aiModel, systemPrompt, collected, tokenAccum, shopName, firstTokenRetriesLeft - 1, logSingleTranslate);
                return;
            }
            ...
            if (isTimeout && pend.length > TIMEOUT_RESPLIT_SIZE) {
                console.warn(`[llm] batch of ${pend.length} timed out (${msg}); re-chunking to ${TIMEOUT_RESPLIT_SIZE}`);
                for (const chunk of chunkArray(pend, TIMEOUT_RESPLIT_SIZE)) {
                    await gatherTranslations(chunk, aiModel, systemPrompt, collected, tokenAccum, shopName, FIRST_TOKEN_DRAIN_RETRIES, logSingleTranslate);
                }
                return;
            }
            const mid = Math.ceil(pend.length / 2);
            console.warn(`[llm] batch of ${pend.length} ${isTimeout ? "timed out" : "unparseable"} (${msg}); splitting`);
            await gatherTranslations(pend.slice(0, mid), aiModel, systemPrompt, collected, tokenAccum, shopName, FIRST_TOKEN_DRAIN_RETRIES, logSingleTranslate);
            await gatherTranslations(pend.slice(mid), aiModel, systemPrompt, collected, tokenAccum, shopName, FIRST_TOKEN_DRAIN_RETRIES, logSingleTranslate);
            return;
        }
...
export function mergeEngineUsage(into, from) {
...
function jsonPlanTexts(plan) {
    const texts = [];
    for (const slot of plan.slotPlans) {
        if (slot.htmlPlan)
            texts.push(...slot.htmlPlan.nodeParts.flat());
        else
            texts.push(slot.text);
    }
    return texts;
}
function listPlanTexts(plan) {
    const texts = [];
    for (const el of plan.elements) {
        if (el.htmlPlan)
            texts.push(...el.htmlPlan.nodeParts.flat());
        else
            texts.push(el.text);
    }
    return texts;
}
function planTextsReady(plan, lookup) {
    const texts = plan.kind === "plain"
        ? plan.parts
        : plan.kind === "html"
            ? plan.nodeParts.flat()
            : plan.kind === "json"
...
function reconstructPlan(plan, rm, lookup, tmWrites, shopName, target, source, skipCacheWrite = false) {
    if (plan.kind === "plain") {
        const pieces = plan.parts.map((p) => lookup(plan.poolSig, p) ?? { value: p, status: "fallback" });
        ...
        const out = plan.nodeParts.map((parts) => {
            const pieces = parts.map((p) => {
                ...
                if (looksLikeWrongScriptLeak(p, r.value, target) ||
        ...
        for (let i = 0; i < plan.slotPlans.length; i++) {
            ...
            if (slot.htmlPlan) {
                ...
                else if (looksLikeWrongScriptLeak(slot.text, r.value, target) ||
        ...
        let anyFallback = false;
        const list = JSON.parse(plan.originalValue);
        const result = [...list];
        for (const el of plan.elements) {
            if (el.htmlPlan) {
...
function logSingleTranslatePath(enabled, kind, details) {
    if (!enabled)
        return;
    console.log(`[single] ${kind}`, details);
}
export async function translateResources(resources, source, target, aiModel, shopName, onProgress, onResourceDone, shouldAbort, options) {
    const abortRequested = async () => shouldAbort ? Boolean(await shouldAbort()) : false;
    ...
    const pools = new Map();
    const addUnit = (order, text, isHandle = false) => {
        if (!isTranslatableLeafText(text))
            return;
    ...
    const skipNonSourceScript = /^(1|true|yes)$/i.test(process.env.TRANSLATE_SKIP_NON_SOURCE_SCRIPT ?? "");
    ...
    for (const res of resources) {
        resultMaps.set(res.resourceId, new Map());
    }
    const fieldWorks = [];
    for (const res of resources) {
    ...
    const cacheHits = skipCacheRead
        ? fieldWorks.map(() => null)
        : await Promise.all(fieldWorks.map(({ f, klass, cacheModel }) => klass === "plain" ? tmGet(shopName, target, cacheModel, f.digest) : Promise.resolve(null)));
    ...
    for (let wi = 0; wi < fieldWorks.length; wi++) {
        ...
        if (!skipCacheRead && klass === "plain") {
            ...
            const cachedByValue = await tmGetByValue(valueCacheSource, source, target, cacheModel, f.digest);
        ...
        const nonSourceScriptSkip = skipNonSourceScript && !containsSourceScript(f.value, source);
    ...
    if (cacheUnits > 0 && onProgress)
        await onProgress(cacheUnits, 0);
    const plansByResource = new Map();
    for (const plan of plans) {
        const list = plansByResource.get(plan.resourceId) ?? [];
        list.push(plan);
        plansByResource.set(plan.resourceId, list);
    }
    const reconstructedResources = new Set();
    const buildResourceOutput = (res) => {
        const rm = resultMaps.get(res.resourceId);
        return {
            resourceId: res.resourceId,
            results: res.fields.map((f) => rm.get(f.key) ?? {
                key: f.key,
    ...
    let finishLock = Promise.resolve();
    const finishReadyResources = async (lookup) => {
        await (finishLock = finishLock.then(async () => {
            if (await abortRequested())
                return;
            for (const res of resources) {
                if (reconstructedResources.has(res.resourceId))
                    continue;
                const resourcePlans = plansByResource.get(res.resourceId);
                if (!resourcePlans) {
                    reconstructedResources.add(res.resourceId);
                    if (onResourceDone)
                        await onResourceDone(buildResourceOutput(res));
                    continue;
                }
                if (!resourcePlans.every((plan) => planTextsReady(plan, lookup)))
                    continue;
                const rm = resultMaps.get(res.resourceId);
    ...
    for (const [sig, occ] of pools) {
        ...
        if (!skipCacheRead) {
            const leafHits = await Promise.all(allTexts.map((text) => tmGetByValue(text, source, target, cacheModel)));
            ...
            if (leafCacheUnits > 0 && onProgress)
                await onProgress(leafCacheUnits, 0);
            if (tmap.size > 0) {
                translated.set(sig, tmap);
                const lookupHit = (poolSig, text) => translated.get(poolSig)?.get(text);
        ...
        const items = texts.map((t, i) => ({ key: String(i), value: t, digest: "" }));
        const { maxChars, maxItems } = resolveBatchLimits(order);
        const batches = batchByChars(items, maxChars, maxItems);
        await Promise.all(batches.map(async (batch) => {
            if (await abortRequested())
                return;
            const { results: m, llmTokens } = await translateItemsRouted(batch, source, target, aiModel, shopName, order, isHandle ? "handle" : "default", customPrompt, logSingleTranslate);
            let batchUnits = 0;
            for (const [k, v] of m) {
            ...
            translated.set(sig, tmap);
            if (onProgress)
                await onProgress(batchUnits, llmTokens);
            const lookup = (poolSig, text) => translated.get(poolSig)?.get(text);
            await finishReadyResources(lookup);
        }));
        translated.set(sig, tmap);
    }
    const retried = await retryPoolFallbacks(translated, pools, source, target, aiModel, shopName, abortRequested, customPrompt, skipCacheWrite
        ? undefined
    ...
    const lookup = (poolSig, text) => translated.get(poolSig)?.get(text);
    await finishReadyResources(lookup);
    if (tmWrites.length > 0)
        await Promise.all(tmWrites);
    // 4. Assemble per-resource results aligned to input field order.
    const out = resources.map((res) => {
        const rm = resultMaps.get(res.resourceId);
        return {
            resourceId: res.resourceId,
            results: res.fields.map((f) => enforceTranslateResultLimits(rm.get(f.key) ?? {
                key: f.key,
                translatedValue: f.value,
                digest: f.digest,
                status: "fallback",
 ...
 */
export async function translateBatch(items, source, target, aiModel, shopName, options) {
    const { resources } = await translateResources([{ resourceId: "__single__", fields: items }], source, target, aiModel, shopName, undefined, undefined, undefined, options);
    return resources[0].results;
}
//# sourceMappingURL=llmTranslate.js.map