...
export function isTitleFieldKey(key) {
 ...
 */
export function clampTitleFieldValue(value, maxChars = SHOPIFY_TITLE_MAX_CHARS) {
...
export function enforceFieldTranslationLimits(key, value) {
    if (!isTitleFieldKey(key))
        return value;
    return clampTitleFieldValue(value);
}
export function enforceTranslateResultLimits(result) {
    const clamped = enforceFieldTranslationLimits(result.key, result.translatedValue);
    if (clamped === result.translatedValue)
        return result;
    return { ...result, translatedValue: clamped };
}
//# sourceMappingURL=translationFieldLimits.js.map