...
async function processInitJob(jobId: string, shopName: string): Promise<void> {
  ...
  try {
    ...
    const fetchOptions = {
      ...
      ...(scopedResourceIds ? { resourceIds: scopedResourceIds } : {}),
    };

    // Trial/expand scoped jobs must stay on paginated ID fetch — bulk enumerates the whole shop.
    const useBulk =
      !scopedResourceIds && isShopInBulkInitAllowlist(shopDomain);
    if (useBulk) {
